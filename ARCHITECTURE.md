...
1. Agent calls: run.sh send --to <addr> --amount <nano>
   ...
   +-- Sign with tweaked Ed25519 (exme_app_key + vanity tweak)
   +-- Encode payload -> Keystone UR format -> QR PNG
   +-- Upload QR to api.exme.org/qr/upload -> CDN URL
...
4. Agent calls: run.sh sign-submit --photo <path>
   ...
   +-- Assemble: app_sig + device_sig + payload -> signed BOC
   +-- Broadcast via api.exme.org proxy -> TON network
...
Recovery Code (71 chars Base58)
  ...
  -> Split: app_secret(12) + tweak(8) + device_pubkey(32)

app_seed = SHA-256(app_secret || "exme-cortex-v1")
...
All TON blockchain requests go through `api.exme.org`:

```
Skill -> api.exme.org/ton/v2/... -> tonapi.io (with server-side API key)
```
