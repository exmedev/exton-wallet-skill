...
Когда пользователь говорит "создай кошелёк", "новый кошелёк", "create wallet":

Recovery Code НЕ нужен — он будет СГЕНЕРИРОВАН автоматически.
Нужна только фотография QR-кода с Keystone 3 Pro.

1. Сказать: "Откройте Keystone 3 Pro → выберите кошелёк Tonkeeper → нажмите на значок экспорта/QR. Сфотографируйте QR-код с экрана Keystone и отправьте фото сюда."
2. Когда пользователь пришлёт фото:
   ```
   exec: bash {baseDir}/scripts/run.sh create-wallet --photo <path_to_photo>
   ```
3. Из JSON ответа показать:
   - Поле `wallet_address` — адрес нового кошелька
   - Поле `recovery_code` — Recovery Code (СГЕНЕРИРОВАН скриптом)
   - Предупреждение: "СОХРАНИТЕ Recovery Code! Запишите на бумаге. Это единственный способ восстановить кошелёк."
4. Кошелёк сразу готов к использованию.
