...
**Шаг 2: Отправить QR-картинку пользователю**
Из JSON ответа возьми `qr_path` — путь к PNG файлу QR-кода.
Отправь его как медиа-сообщение через OpenClaw CLI:
```
exec: openclaw message send --media <qr_path> --message "📱 Подпишите на Keystone 3 Pro:\n1. TON → Scan → отсканируйте QR\n2. Проверьте детали → подтвердите\n3. Сфотографируйте QR подписи\n4. Отправьте фото в этот чат"
```

Это отправит QR-картинку напрямую в чат (Telegram/WhatsApp/Discord).
